...
class DSMPCF(nn.Module):
    def __init__(self, inplanes , dsm_channel ,scale):
        super( DSMPCF, self).__init__()

        self.cutConv = nn.Conv2d(  inplanes  ,  64  , kernel_size=3 , padding=1   , stride=2 ,bias=False)

        self.block1 = dsm_block( 64 , dsm_channel[0] , scale)
        self.block2 = dsm_block( dsm_channel[0] , dsm_channel[1] , scale )
        self.block3 = dsm_block( dsm_channel[1] , dsm_channel[2] , scale)
        self.block4 = dsm_block( dsm_channel[2] , dsm_channel[3] , scale)
...
#         print( self.dsm_channel )
        self.dsmpcf = DSMPCF( 1 , self.dsm_channel ,scale=4 )
