...
class dsm_block(nn.Module):
    def __init__(self, inplanes , out_c ,scale, num_head):
...
#             self.flow.add_module("r2_" + str(i), R2(  out_c  ,  out_c  , stride=1 , scale=self.scale ))
        
#         self.flow.add_module("conv",nn.Conv2d(  out_c  ,  out_c  , kernel_size=3 , stride=1 , padding=1 ,bias=False))
        
#         self.flow.add_module("conv_2",nn.Conv2d(  out_c  ,  out_c  , kernel_size=3 , padding=1 ,groups=out_c  , stride=2 ,bias=False))
...
class DSMPCF(nn.Module):
    def __init__(self, inplanes , dsm_channel , num_heads,scale):
        ...
        self.block1 = dsm_block( 64 , dsm_channel[0] , scale, num_heads[0])
        self.block2 = dsm_block( dsm_channel[0] , dsm_channel[1] , scale, num_heads[1])
        self.block3 = dsm_block( dsm_channel[1] , dsm_channel[2] , scale , num_heads[2] )
        self.block4 = dsm_block( dsm_channel[2] , dsm_channel[3] , scale , num_heads[3])
...
class FusionAttention(nn.Module):
    ...
    def __init__(self, channel):
        ...
        self.ssa = SharedSpatialAttention(channel)

    def forward(self, x):
        
        out = self.sca(x) + self.ssa(x)
...
class RFNet(nn.Module):
    ...
    def __init__(self,
        ...
        for j in range(self.num_layers):
            layer = FusionAttention(embed_dim*(2**j))
            
            self.fa.append(layer)
        
        self.dsm_channel = [ embed_dim*(2**i) for i in range(0,4)]
#         print( self.dsm_channel )
        self.dsmpcf = DSMPCF( 1 , self.dsm_channel , num_heads ,scale=4 )
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                out =  x_out.view(-1, H, W, self.num_features[i]).permute(0, 3, 1, 2).contiguous() # swin
                # out =  fa_layer( x_out.view(-1, H, W, self.num_features[i]).permute(0, 3, 1, 2).contiguous() + dsm_x[i]) # dcfanet
                # out =  x_out.view(-1, H, W, self.num_features[i]).permute(0, 3, 1, 2).contiguous() + dsm_x[i] # dcfanet 消融
