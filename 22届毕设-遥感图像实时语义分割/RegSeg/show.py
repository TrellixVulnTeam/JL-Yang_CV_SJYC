...
def get_colors_mapillary():
    ...
    colors = [[165, 42, 42], [0, 192, 0], [196, 196, 196], [190, 153, 153], [180, 165, 180], [90, 120, 150],
              ...
              [220, 128, 128], [222, 40, 40], [100, 170, 30], [40, 40, 40], [33, 33, 33], [100, 128, 160], [142, 0, 0],
...
def get_colors_mapillary_reduced():
    colors = [[165, 42, 42], [0, 192, 0], [196, 196, 196], [190, 153, 153], [180, 165, 180], [90, 120, 150],
              ...
              [120, 10, 10], [0, 0, 0]]
    colors = np.array(colors).astype("uint8")
    ious = [0.0, 0.0, 57.68, 58.66, 63.16, 56.59, 50.9, 45.04, 39.82, 18.31, 22.22, 45.77, 49.91, 87.97, 43.31, 70.61,
            76.67, 86.43, 41.84, 66.81, 46.77, 50.41, 0.0, 69.53, 57.07, 48.28, 4.99, 97.77, 76.83, 68.69, 88.77, 72.93,
            17.02, 22.26, 5.05, 45.31, 29.76, 0.0, 20.38, 36.26, 2.43, 43.12, 4.4, 0.0, 37.03, 40.48, 52.69, 44.16,
            60.96, 36.36, 66.61, 43.94, 47.44, 16.69, 73.89, 89.68, 0.0, 55.74, 46.28, 22.28, 6.71, 67.39, 8.41, 68.79,
            91.75, 0]
    ious = np.array(ious)
...
def dispay_mapillary():
    ...
    with open(config_filename) as file:
        config = yaml.full_load(file)
    train_loader, val_loader, train_set = get_dataset_loaders(config)
    display(train_loader, lambda images: show_mapillary_mask(images, False), num_images=num_images, skip=skip,
...
def show_mapillary_model():
    ...
    np.random.seed(seed)
    train_loader, val_loader, train_set = get_dataset_loaders(config)
    show(model, train_loader, device, show_mapillary_mask, num_images=num_images, skip=skip,
         images_per_line=images_per_line)
...
def show_cityscapes_failure_modes():
    ...
    indices = [312, 84, 54, 161, 230, 319, 303, 297, 262, 310, 263, 318, 360, 11, 321, 290, 257, 283, 121, 101, 276,
               100, 317, 124, 175, 267, 278, 178, 231, 228]
