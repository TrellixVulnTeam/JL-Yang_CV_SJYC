...
from evaluation import MscEvalV1
...
def train():
    ...
    cropsize = [896, 896]
    randomscale = (
        0.5, 0.625, 0.75, 0.875, 1.0, 1.125, 1.25, 1.375, 1.5, 1.625, 1.75, 1.875, 2.0, 2.125, 2.25, 2.375, 2.5)
    ...
    ds = Potsdam(dspth, cropsize=cropsize, mode=mode, randomscale=randomscale)
    ...
    dl = DataLoader(ds,
                    ...
                    drop_last=True)
    # exit(0)
    dsval = Potsdam(dspth, mode='val', randomscale=randomscale)
    ...
    for it in range(max_iter):
        ...
        if (it + 1) % msg_iter == 0:
            ...
            loss_boundery_bce_avg = sum(
                loss_boundery_bce) / len(loss_boundery_bce)
            loss_boundery_dice_avg = sum(
                loss_boundery_dice) / len(loss_boundery_dice)
            msg = ', '.join([
                'it: {it}/{max_it}',
                'lr: {lr:4f}',
                'loss: {loss:.4f}',
            ...
            writer.add_scalar("Loss/train", loss_avg, it + 1)
            writer.add_scalar("Loss_bce/train", loss_boundery_bce_avg, it + 1)
            writer.add_scalar("Loss_dice/train", loss_boundery_dice_avg, it + 1)
        ...
        if (it + 1) % save_iter_sep == 0:  # and it != 0:
            ...
            with torch.no_grad():
                single_scale1 = MscEvalV1()
                mIOU50 = single_scale1(net, dlval, n_classes)

                single_scale2 = MscEvalV1(scale=0.75)
                mIOU75 = single_scale2(net, dlval, n_classes)

                single_scale3 = MscEvalV1(scale=1.0)
            ...
            writer.add_scalars("mIOU/eval", {"mIOU50": mIOU50, "mIOU75": mIOU75, "mIOU100": mIOU100}, it + 1)
            ...
            logger.info(
                'training iteration {}, model saved to: {}'.format(it + 1, save_pth))

            if mIOU50 > maxmIOU50:
                maxmIOU50 = mIOU50
                save_pth = osp.join(
                    save_pth_path, 'model_maxmIOU50.pth'.format(it + 1))
                state = net.module.state_dict() if hasattr(net, 'module') else net.state_dict()
                if dist.get_rank() == 0:
                    torch.save(state, save_pth)
            ...
            if mIOU75 > maxmIOU75:
                maxmIOU75 = mIOU75
                save_pth = osp.join(
                    save_pth_path, 'model_maxmIOU75.pth'.format(it + 1))
                state = net.module.state_dict() if hasattr(net, 'module') else net.state_dict()
                if dist.get_rank() == 0:
                    torch.save(state, save_pth)
                logger.info('max mIOU model saved to: {}'.format(save_pth))
            ...
            if mIOU100 > maxmIOU100:
                maxmIOU100 = mIOU100
                save_pth = osp.join(
                    save_pth_path, 'model_maxmIOU100.pth'.format(it + 1))
                state = net.module.state_dict() if hasattr(net, 'module') else net.state_dict()
                if dist.get_rank() == 0:
                    torch.save(state, save_pth)
                logger.info('max mIOU model saved to: {}'.format(save_pth))
            ...
            logger.info('maxmIOU50 is: {}, maxmIOU75 is: {}, maxmIOU100 is: {}.'.format(
                maxmIOU50, maxmIOU75, maxmIOU100))
