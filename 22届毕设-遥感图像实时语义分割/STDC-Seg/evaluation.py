...
class MscEvalV0(object):
    ...
    def __call__(self, net, dl, n_classes):
        ...
        for i, (imgs, label) in diter:
            ...
            label_cpu = label.cpu()
            pred_cpu = preds.cpu()
            label_np = label_cpu[keep].numpy()
            pred_np = pred_cpu[keep].numpy()
            label_np_1d = label_np.flatten()
            pred_np_1d = pred_np.flatten()
            oa.append(metrics.accuracy_score(label_np_1d, pred_np_1d))
            hist += torch.bincount(
                label[keep] * n_classes + preds[keep],
                minlength=n_classes ** 2
            ).view(n_classes, n_classes).float()
        ...
        miou = ious.mean()
        oa = np.array(oa)
        return miou.item(), np.average(oa)


def evaluatev0(respth='./pretrained', dspth='./data/potsdam', backbone='CatNetSmall', scale=0.75, use_boundary_2=False,
    ...
    with torch.no_grad():
        ...
        mIOU, oa = single_scale(net, dl, 6)
    logger = logging.getLogger()
    logger.info('mIOU is: %s\n', mIOU)
    logger.info('os is: {}'.format(oa))
...
if __name__ == "__main__":
    ...
    evaluatev0('./checkpoints/train_STDC2-SEG-6000ITR/pths/model_maxmIOU50.pth', dspth='./data/potsdam',
               backbone='STDCNet1446',
               scale=0.5,
               use_boundary_2=False, use_boundary_4=False, use_boundary_8=True, use_boundary_16=False)

    # STDC2-Seg75 mIoU 0.7704
    evaluatev0('./checkpoints/train_STDC2-SEG-6000ITR/pths/model_maxmIOU75.pth', dspth='./data/potsdam',
               backbone='STDCNet1446',
               scale=0.75,
               use_boundary_2=False, use_boundary_4=False, use_boundary_8=True, use_boundary_16=False)
